...
Los siguientes modelos fueron implementados en orden cronológico para entender bien el contexto de la competición y el funcionamiento de las clases. Los productos de esta fase son **EMERALDS** (fair value fijo en 10.000) y **TOMATOES** (drift lento, spread visible amplio de ~14 ticks). Position limit: 80 por producto.
...
- **`model_v2.py`** — tres cambios concretos sobre v1, derivados de un análisis de los trades externos a nivel de fill:
  ...
  3. `TOMATOES_MAX_PASSIVE_SIZE = 12` (antes 5) — ~2 trades externos por snapshot con ~3.5 unidades promedio cada uno (~7 unidades de exposición total por ventana). Size 5 se maxeaba en cada snapshot.
...
python models_tutorial/backtest_selected_model.py --model model_v2 --days -2 -1
```

Los plots y CSVs se guardan en `models_tutorial/backtest_plots/<model_name>/`.
