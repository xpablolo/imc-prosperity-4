...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_8: best PEPPER refinement plus tiny ASH execution polish.
    ...
    LATE_SIGNAL_BIAS = 0.60
    EARLY_LONG_FLOOR = 76
    MID_LONG_FLOOR = 64
    LATE_LONG_FLOOR = 66
    ENDGAME_LONG_FLOOR = 44

    UNDERINVESTED_GAP_GUARD = 9
    UNDERINVESTED_SELL_SCALE = 0.30
    HYSTERESIS_LATE_FLOOR_BONUS = 5

    NEUTRAL_L2_BAND = 0.06
    NEUTRAL_CONT_BAND = 0.16
    NEUTRAL_MICRO_BAND = 0.10
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            if progress < self.EARLY_END:
                ...
                bonus_scale = 0.46
                sell_scale = 0.42
            bonus = int(round(min(10, gap / 7.0 * bonus_scale)))
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + bonus)
            shortfall_frac = min(1.0, gap / max(1.0, target_inventory))
            passive_sell_size = int(math.floor(passive_sell_size * max(0.0, 1.0 - shortfall_frac * (1.0 - sell_scale))))
            if gap >= self.UNDERINVESTED_GAP_GUARD:
                passive_sell_size = int(math.floor(passive_sell_size * self.UNDERINVESTED_SELL_SCALE))
    ...
    def adjust_directional_quotes(
        ...
        raw_signal = trend_signal + 0.8 * continuation_adj + self._inventory_bias()
        neutral_book = (
            abs(l2_imbalance) < self.NEUTRAL_L2_BAND
            and abs(continuation_adj) < self.NEUTRAL_CONT_BAND
            and abs(micro_shift) < self.NEUTRAL_MICRO_BAND
        )
        intention = 0
        ...
        if raw_signal > -0.18 and gap > 0:
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if ((intention >= 3 and gap >= 10) or (gap >= 18 and not neutral_book)) else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END:
                passive_ask += 1
            if progress < self.EARLY_END and (gap >= 10 or intention >= 3):
                passive_ask += 1
            if intention >= 3:
                buy_bonus = 3 if gap >= 16 else 2
                sell_scale = 0.10 if progress < self.MID_END else 0.32
            elif neutral_book:
                buy_bonus = 1
                sell_scale = 0.24 if progress < self.MID_END else 0.54
            else:
                buy_bonus = 2
                sell_scale = 0.16 if progress < self.MID_END else 0.42
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + buy_bonus)
            passive_sell_size = int(math.floor(passive_sell_size * sell_scale))
...
class _AshTraderTiny(_AshTrader):
    """Tiny ASH execution polish: slightly more tolerant marginal repair fills."""

    MARGINAL_TAKE_TOLERANCE = 0.60
    DIRECTIONAL_SIZE_BONUS = 3


class Trader:
    """Round 1 model_E: keep ASH stable, push harder on PEPPER drift capture."""

    def __init__(self) -> None:
        self.ash = _AshTraderTiny()
