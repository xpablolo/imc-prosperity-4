...
class _PepperTrader(_PepperTraderBase):
    """F2: PEPPER time-scheduled target inventory."""
    ...
    def _scheduled_target(self, l2_imbalance: float, flow_recent: float) -> int:
        ...
        if l2_imbalance < -0.20 and flow_recent < -0.12:
            target -= 10
        elif l2_imbalance < -0.10:
            target -= 4
        elif l2_imbalance > 0.08 and flow_recent > -0.05 and p < self.LATE_END:
            target += 2
        return max(10, min(self.TARGET_INVENTORY_MAX, target))
    ...
    def desired_inventory(self, trend_signal: float, continuation_adj: float, visible_spread: int | None, vol: float, l2_imbalance: float, flow_recent: float) -> int:
        ...
        if raw_signal < -0.75:
            target = max(8, target - 14)
        elif raw_signal < -0.25:
            target = max(10, target - 8)
        self._last_schedule_target = target
        return target
    ...
    def carry_inventory_floor(self, trend_signal: float, continuation_adj: float, l2_imbalance: float, flow_recent: float, target_inventory: int) -> int:
        floor = max(8, int(round(self._scheduled_target(l2_imbalance, flow_recent) * 0.86)))
        if trend_signal + 0.8 * continuation_adj < -0.85:
            floor = max(8, floor - 12)
    ...
    def passive_sizes(self, position: int, target_inventory: int, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int, vol: float, visible_spread: int | None, trend_signal: float, continuation_adj: float) -> tuple[int, int]:
        ...
        if gap > 0:
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + min(4, max(1, gap // 10)))
            passive_sell_size = int(math.floor(passive_sell_size * (0.25 if gap >= 12 else 0.55)))
        return max(0, passive_buy_size), max(0, passive_sell_size)
...
class Trader:
    """Round 1 model_F2: new PEPPER strategy idea variant."""
