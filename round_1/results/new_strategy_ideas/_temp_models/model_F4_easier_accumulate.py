...
class _PepperTrader(_PepperTraderBase):
    """F4: PEPPER state machine / regime controller."""
    ...
    def _state_label(self, position: int, l2_imbalance: float, continuation_adj: float, flow_recent: float) -> str:
        p = self._session_progress()
        if p < self.EARLY_END and position < 64:
            return 'ACCUMULATE'
        if position < 60 and (l2_imbalance > -0.08 or continuation_adj > -0.12):
            return 'ACCUMULATE'
        if p >= self.LATE_END and position > 64 and l2_imbalance < -0.10:
            return 'REDUCE'
    ...
    def desired_inventory(self, trend_signal: float, continuation_adj: float, visible_spread: int | None, vol: float, l2_imbalance: float, flow_recent: float) -> int:
        state = getattr(self, '_policy_state', 'CARRY')
        target = {
            'ACCUMULATE': self.ACCUMULATE_TARGET,
            'CARRY': self.CARRY_TARGET,
            'DEFEND': self.DEFEND_TARGET,
            'REDUCE': self.REDUCE_TARGET,
        }[state]
        raw_signal = trend_signal + 0.8 * continuation_adj
        if raw_signal < -0.75:
            target -= 10
        elif raw_signal > 0.60 and state in {'ACCUMULATE', 'CARRY'}:
            target += 4
        self._last_state_target = max(8, min(self.TARGET_INVENTORY_MAX, target))
    ...
    def carry_inventory_floor(self, trend_signal: float, continuation_adj: float, l2_imbalance: float, flow_recent: float, target_inventory: int) -> int:
        state = getattr(self, '_policy_state', 'CARRY')
        if state == 'ACCUMULATE':
            return max(24, target_inventory - 10)
        if state == 'CARRY':
            return max(20, int(round(target_inventory * 0.82)))
        if state == 'DEFEND':
            return max(14, int(round(target_inventory * 0.74)))
        return max(8, int(round(target_inventory * 0.60)))
    ...
    def passive_sizes(self, position: int, target_inventory: int, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int, vol: float, visible_spread: int | None, trend_signal: float, continuation_adj: float) -> tuple[int, int]:
        ...
        if state == 'ACCUMULATE':
            ...
            passive_sell_size = int(math.floor(passive_sell_size * 0.15))
        elif state == 'CARRY':
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
            passive_sell_size = int(math.floor(passive_sell_size * 0.42))
        elif state == 'DEFEND':
            passive_sell_size = int(math.floor(passive_sell_size * 0.75))
        elif state == 'REDUCE':
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state_store: Dict, market_trades: List[Trade], timestamp: int) -> List[Order]:
        ...
        pep_state = state_store.setdefault(self.PRODUCT, {})
        flow_recent = float(pep_state.get('flows', [])[-1]) if pep_state.get('flows') else 0.0
        cont = flow_recent * self.CONTINUATION_BONUS if abs(flow_recent) >= self.CONTINUATION_IMBALANCE else 0.0
        state = self._state_label(position, l2, cont, flow_recent)
        ...
        try:
            ...
            self._policy_state = 'CARRY'


class Trader:
    """Round 1 model_F4: new PEPPER strategy idea variant."""
