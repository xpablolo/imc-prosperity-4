...
class _PepperTrader(_PepperTraderBase):
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            ...
            bonus = int(round(min(8, (1.1 + 5.8 * gap_ratio) * bonus_scale)))
    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if intention >= 3 and gap >= self.MICRO_PLACEMENT_GAP else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and gap >= self.MICRO_PLACEMENT_GAP:
                passive_ask += 1
            if intention >= 3:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (2 if gap >= 16 else 1))
...
class Trader:
    """Round 1 model_E_plus_14: PEPPER placement + size micro-refinement."""
