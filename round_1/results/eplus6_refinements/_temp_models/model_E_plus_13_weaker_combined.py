...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_6: PEPPER book-intention aggression modulation.

    Philosophy:
    - PEPPER alpha comes mainly from carrying long inventory through the drift.
    - Time, imbalance, and residuals should mostly modulate buy aggression and
      reduce premature selling, not create fair-value flip-flopping.
    - Keep ASH unchanged; only push harder on PEPPER carry capture.
    """

    # Start participating a bit earlier, but without forcing a price overlay.
    WARMUP_TICKS = 10

    # Allow a meaningfully larger long warehouse, still inside the hard limit.
    TARGET_INVENTORY_MAX = 80
    SOFT_POSITION_LIMIT = 74
    HARD_POSITION_LIMIT = 80
    MAX_PASSIVE_SIZE = 22
    PASSIVE_REDUCTION_STEP = 5

    # Keep more budget available for trend warehousing.
    BUDGET_MIN_UNITS = 72
    BUDGET_MAX_UNITS = 80
    BUDGET_BASE_RISK = 700.0

    # Enter carry / warehouse mode earlier and resist trimming too easily.
    CARRY_MODE_SIGNAL = 1.20
    CARRY_MODE_IMBALANCE = -0.02
    CARRY_MODE_FLOW_FLOOR = -0.18
    CARRY_FLOOR_FRACTION = 0.68
    CARRY_OPPOSITE_SIZE_SCALE = 0.18
    CARRY_EXTRA_QUOTE_WIDEN = 2
    CARRY_SIZE_BONUS = 3

    WAREHOUSE_MODE_SIGNAL = 1.95
    WAREHOUSE_MODE_IMBALANCE = 0.00
    WAREHOUSE_MODE_FLOW = -0.12
    WAREHOUSE_TARGET_FRACTION = 0.94
    WAREHOUSE_FLOOR_FRACTION = 1.0
    WAREHOUSE_THRESHOLD_SHIFT = 1
    WAREHOUSE_SIZE_BONUS = 3
    WAREHOUSE_OPPOSITE_SCALE = 0.18

    # Trim less aggressively on small positive residuals.
    STRETCH_Z_THRESHOLD = 1.90
    STRETCH_TRIM_BONUS = 0.25

    # PEPPER should be strongly asymmetric: strong bids, weak asks.
    DIRECTIONAL_OPPOSITE_SIZE_SCALE = 0.30
    TOXIC_OPPOSITE_SIZE_SCALE = 0.45

    # Mild structural long bias by session phase.
    EARLY_END = 0.30
    MID_END = 0.68
    LATE_END = 0.88
    EARLY_SIGNAL_BIAS = 1.52
    MID_SIGNAL_BIAS = 0.90
    LATE_SIGNAL_BIAS = 0.52
    EARLY_LONG_FLOOR = 76
    MID_LONG_FLOOR = 64
    LATE_LONG_FLOOR = 58
    ENDGAME_LONG_FLOOR = 36

    UNDERINVESTED_GAP_GUARD = 10
    UNDERINVESTED_SELL_SCALE = 0.35
    HYSTERESIS_LATE_FLOOR_BONUS = 4

    INTENTION_STRONG_L2 = 0.12
    INTENTION_SOFT_L2 = 0.05
    INTENTION_MICRO = 0.10
    INTENTION_CONT = 0.16
    PLACEMENT_GAP_SOFT = 10
    PLACEMENT_GAP_STRONG = 16
    HYSTERESIS_NEUTRAL_SIGNAL = 0.06
    HYSTERESIS_STRONG_INTENTION = 3
    INTENTION_NEEDS_CONFIRM = 2
    INTENTION_STRONG_CONFIRM = 4
    ...
    def _inventory_bias(self) -> float:
        ...
        if p < self.EARLY_END:
            return self.EARLY_SIGNAL_BIAS
        if p < self.MID_END:
            span = (p - self.EARLY_END) / max(1e-9, self.MID_END - self.EARLY_END)
            return self.EARLY_SIGNAL_BIAS + (self.MID_SIGNAL_BIAS - self.EARLY_SIGNAL_BIAS) * span
        if p < self.LATE_END:
            span = (p - self.MID_END) / max(1e-9, self.LATE_END - self.MID_END)
            return self.MID_SIGNAL_BIAS + (self.LATE_SIGNAL_BIAS - self.MID_SIGNAL_BIAS) * span
        return 0.0

    def _session_long_floor(self, l2_imbalance: float, flow_recent: float) -> int:
        p = self._session_progress()
        if p < self.EARLY_END:
            floor = self.EARLY_LONG_FLOOR
        elif p < self.MID_END:
            floor = self.MID_LONG_FLOOR
        elif p < self.LATE_END:
            floor = self.LATE_LONG_FLOOR
        else:
            floor = self.ENDGAME_LONG_FLOOR

        # Only back off if both book and flow are genuinely adverse.
        if l2_imbalance < -0.26 and flow_recent < -0.26:
            floor -= 14
        elif l2_imbalance < -0.18 and flow_recent < -0.16:
            floor -= 8
        elif l2_imbalance < -0.12:
            floor -= 4
        if p >= self.MID_END:
            floor += self.HYSTERESIS_LATE_FLOOR_BONUS
        return max(6, floor)
    ...
    def desired_inventory(
        ...
        trend_signal: float,
        continuation_adj: float,
        visible_spread: int | None,
        vol: float,
        l2_imbalance: float,
        flow_recent: float,
    ) -> int:
        base_target = super().desired_inventory(
            trend_signal,
            continuation_adj,
            visible_spread,
            vol,
        ...
        raw_signal = trend_signal + 0.8 * continuation_adj + self._inventory_bias()
        progress = self._session_progress()
        long_floor = self._session_long_floor(l2_imbalance, flow_recent)

        if raw_signal > -0.35 and progress < self.LATE_END:
            boosted = max(0, base_target)
            strength_bonus = 1.10 + 0.12 * max(0.0, (self.MID_END - progress) / self.MID_END)
            boosted = int(round(min(self.TARGET_INVENTORY_MAX, boosted * strength_bonus)))
            if l2_imbalance > 0.08:
                boosted = min(self.TARGET_INVENTORY_MAX, boosted + 4)
            elif l2_imbalance > -0.04:
                boosted = min(self.TARGET_INVENTORY_MAX, boosted + 2)
            return max(boosted, long_floor)
    ...
    def inventory_budget_units(
        ...
        units = super().inventory_budget_units(
            vol,
            visible_spread,
            resid_z,
            trend_signal,
        ...
        if raw_signal > 0.0:
            if progress < self.EARLY_END:
                units += 8
            elif progress < self.MID_END:
                units += 5
            elif progress < self.LATE_END:
                units += 2
        return int(round(self.clamp(units, self.BUDGET_MIN_UNITS, self.BUDGET_MAX_UNITS)))
    ...
    def carry_inventory_floor(
        ...
        base_floor = super().carry_inventory_floor(
            trend_signal,
            continuation_adj,
            l2_imbalance,
            flow_recent,
        ...
        progress = self._session_progress()
        floor = base_floor
        if raw_signal > -0.55 and progress < 0.98:
            floor = max(floor, self._session_long_floor(l2_imbalance, flow_recent))
        if target_inventory > 0 and progress < 0.96 and raw_signal > -0.35:
            floor = max(floor, max(0, target_inventory - self.UNDERINVESTED_GAP_GUARD))
        return floor
    ...
    def passive_sizes(
        ...
        passive_buy_size, passive_sell_size = super().passive_sizes(
            position,
            target_inventory,
            buy_capacity,
            sell_capacity,
        ...
        gap = target_inventory - position
        self._inventory_gap_snapshot = gap
        progress = self._session_progress()
        if target_inventory > 0 and gap > 0:
            if progress < self.EARLY_END:
                bonus_scale = 1.00
                sell_scale = 0.22
            elif progress < self.MID_END:
                ...
                bonus_scale = 0.44
                sell_scale = 0.50
            gap_ratio = min(1.0, gap / max(1.0, target_inventory))
            bonus = int(round(min(8, (1.5 + 6.0 * gap_ratio) * bonus_scale)))
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + bonus)
            passive_sell_size = int(math.floor(passive_sell_size * max(0.0, 1.0 - gap_ratio * (1.0 - sell_scale))))
            if gap >= self.UNDERINVESTED_GAP_GUARD:
                passive_sell_size = int(math.floor(passive_sell_size * (self.UNDERINVESTED_SELL_SCALE + 0.02)))
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
        return max(0, passive_buy_size), max(0, passive_sell_size)
    ...
    def adjust_directional_quotes(
        ...
        passive_bid, passive_ask, passive_buy_size, passive_sell_size = super().adjust_directional_quotes(
            passive_bid,
            passive_ask,
            passive_buy_size,
            passive_sell_size,
        ...
        progress = self._session_progress()
        gap = max(0, int(getattr(self, "_inventory_gap_snapshot", 0)))
        raw_signal = trend_signal + 0.8 * continuation_adj + self._inventory_bias()
        intention = 0
        if l2_imbalance > self.INTENTION_SOFT_L2:
            intention += 1
        if l2_imbalance > self.INTENTION_STRONG_L2:
            intention += 1
        if micro_shift > self.INTENTION_MICRO:
            intention += 1
        if continuation_adj > self.INTENTION_CONT:
            intention += 1
        if visible_spread is not None and visible_spread <= 10:
            intention += 1

        if raw_signal > -0.20 and gap > 0:
            supportive_book = intention >= self.INTENTION_NEEDS_CONFIRM
            strong_support = intention >= self.INTENTION_STRONG_CONFIRM
            if abs(raw_signal) <= self.HYSTERESIS_NEUTRAL_SIGNAL and not strong_support and gap < self.PLACEMENT_GAP_STRONG:
                return max(0, passive_bid), max(0, passive_ask), max(0, passive_buy_size), max(0, passive_sell_size)
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if strong_support and gap >= self.PLACEMENT_GAP_SOFT else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and (supportive_book or gap >= self.PLACEMENT_GAP_STRONG):
                passive_ask += 1
            if strong_support:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= self.PLACEMENT_GAP_STRONG else 2))
                passive_sell_size = int(math.floor(passive_sell_size * (0.12 if progress < self.MID_END else 0.38)))
            elif supportive_book:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
                passive_sell_size = int(math.floor(passive_sell_size * (0.20 if progress < self.MID_END else 0.48)))
            else:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
                passive_sell_size = int(math.floor(passive_sell_size * (0.32 if progress < self.MID_END else 0.60)))
    ...
    def trade_pepper(
        ...
        self._inventory_gap_snapshot = 0
        try:
...
class Trader:
    """Round 1 model_E_plus_13: combined simple PEPPER execution refinement on top of E_plus_6."""
