...
class _PepperTrader(_PepperTraderBase):
    """G3: F3 + F2 hybrid policy (aggressive carry with simple schedule support)."""
    ...
    MAX_PASSIVE_SIZE = 24
    PASSIVE_REDUCTION_STEP = 4
    BUDGET_MIN_UNITS = 78
    BUDGET_MAX_UNITS = 80
    BUDGET_BASE_RISK = 740.0
    ...
    SCHEDULE_LATE = 73
    SCHEDULE_END = 54
    HOLD_FLOOR_FRACTION = 0.90

    def _session_progress(self) -> float:
        return float(getattr(self, '_current_progress', 1.0))

    def _schedule_target(self, l2_imbalance: float, flow_recent: float) -> int:
        p = self._session_progress()
        if p < self.EARLY_END:
            target = self.SCHEDULE_EARLY
        elif p < self.MID_END:
            target = self.SCHEDULE_MID
        elif p < self.LATE_END:
            target = self.SCHEDULE_LATE
        else:
            target = self.SCHEDULE_END
    ...
    def desired_inventory(self, trend_signal: float, continuation_adj: float, visible_spread: int | None, vol: float, l2_imbalance: float, flow_recent: float) -> int:
        raw_signal = trend_signal + 0.8 * continuation_adj
        scheduled = self._schedule_target(l2_imbalance, flow_recent)
        hold_target = scheduled + (2 if raw_signal > 0.4 and self._session_progress() < self.LATE_END else 0)
        if raw_signal < -1.0 and l2_imbalance < -0.18:
            hold_target -= 12
        elif raw_signal < -0.50:
            hold_target -= 6
        self._schedule_target_snapshot = scheduled
        self._hold_target_snapshot = max(12, min(self.TARGET_INVENTORY_MAX, hold_target))
        return self._hold_target_snapshot
    ...
    def carry_inventory_floor(self, trend_signal: float, continuation_adj: float, l2_imbalance: float, flow_recent: float, target_inventory: int) -> int:
        floor = max(16, int(round(max(int(getattr(self, '_schedule_target_snapshot', 0)), target_inventory) * self.HOLD_FLOOR_FRACTION)))
        if trend_signal + 0.8 * continuation_adj < -1.0:
            floor = max(12, floor - 10)
    ...
    def passive_sizes(self, position: int, target_inventory: int, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int, vol: float, visible_spread: int | None, trend_signal: float, continuation_adj: float) -> tuple[int, int]:
        ...
        if gap > 0:
            bonus = min(6, max(2, gap // 9))
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + bonus)
            passive_sell_size = int(math.floor(passive_sell_size * (0.14 if gap >= 10 else 0.28)))
        return max(0, passive_buy_size), max(0, passive_sell_size)
    ...
    def adjust_directional_quotes(self, passive_bid: int, passive_ask: int, passive_buy_size: int, passive_sell_size: int, best_bid: int | None, best_ask: int | None, visible_spread: int | None, signal_ticks: float, trend_signal: float, l2_imbalance: float, micro_shift: float, continuation_adj: float, flow_recent: float, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int) -> tuple[int, int, int, int]:
        ...
        gap = max(0, int(getattr(self, '_hold_target_snapshot', 0)) - int(getattr(self, '_position_snapshot', 0)))
        if gap >= 8 and best_bid is not None and best_ask is not None and visible_spread is not None and visible_spread >= 4:
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state_store: Dict, market_trades: List[Trade], timestamp: int) -> List[Order]:
        ...
        self._schedule_target_snapshot = 0
        self._hold_target_snapshot = 0
        try:
...
class Trader:
    """Round 1 model_G3: policy architecture research variant."""
