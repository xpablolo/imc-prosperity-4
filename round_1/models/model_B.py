...
class _PepperTrader:
    ...
    TARGET_INVENTORY_MAX = 38
    INVENTORY_SKEW = 0.14
    SOFT_POSITION_LIMIT = 50
    HARD_POSITION_LIMIT = 74
    MAX_PASSIVE_SIZE = 18
    ...
    CARRY_MODE_SIGNAL = 1.80
    CARRY_MODE_IMBALANCE = 0.12
    CARRY_MODE_FLOW_FLOOR = -0.02
    CARRY_FLOOR_FRACTION = 0.46
    CARRY_OPPOSITE_SIZE_SCALE = 0.42
    CARRY_EXTRA_QUOTE_WIDEN = 1
    CARRY_SIZE_BONUS = 2

    BUDGET_MIN_UNITS = 24
    BUDGET_MAX_UNITS = 64
    BUDGET_BASE_RISK = 470.0
    BUDGET_TREND_BONUS = 0.62
    BUDGET_ADVERSE_VOL = 1.75
    BUDGET_ADVERSE_RESID = 2.30
    BUDGET_ADVERSE_SPREAD = 0.24
    ...
    def inventory_budget_units(
        ...
        aligned_strength = (
            abs(trend_signal)
            + 0.90 * abs(continuation_adj)
            + 1.80 * aligned_imbalance
            + 1.10 * aligned_flow
        )
        risk_budget = self.BUDGET_BASE_RISK * (
            1.0 + self.BUDGET_TREND_BONUS * self.clamp(aligned_strength / 4.0, 0.0, 1.0)
        )
        adverse_move = 3.5 + self.BUDGET_ADVERSE_VOL * vol + self.BUDGET_ADVERSE_RESID * max(0.0, abs(resid_z) - 0.45)
    ...
    def trade_pepper(
        ...
        target_inventory = target_direction * min(inventory_budget, target_abs + int(round(0.45 * extra_budget)))
        ...
        aligned_direction = self.sign(trend_signal + 0.8 * continuation_adj)
        if (
            aligned_direction > 0
            and l2_imbalance > 0.08
            and flow_recent > -0.10
            and abs(trend_signal) + abs(continuation_adj) >= 2.0
        ):
            aggressive_buy_threshold += 1
            aggressive_sell_threshold += 1
        elif (
            aligned_direction < 0
            and l2_imbalance < -0.08
            and flow_recent < 0.10
...
class Trader:
    """
    Round 1 combined model.
    Single self-contained file: no external imports beyond datamodel.

    - ASH_COATED_OSMIUM     → _AshTrader    (stationary MM)
    - INTARIAN_PEPPER_ROOT  → _PepperTrader (trend + pullback)
    """
    ...
    def run(self, state: TradingState):
        ...
        if ash_depth is not None:
            ...
            result[self.ash.PRODUCT] = self.ash.trade_ash(
                self.ash.PRODUCT, ash_depth, ash_position, anchor_fair
            )
        else:
