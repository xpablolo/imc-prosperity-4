...
class _PepperTrader(_PepperTraderBase):
    """
    Research-aligned PEPPER extension of model_C.

    Core changes vs model_C:
    - Blend the learned fair with an explicit linear session fair because the
      public research shows PEPPER follows an almost deterministic intraday slope.
    - Keep a persistent long baseline instead of allowing target inventory to
      swing to flat / short too easily.
    - Let supportive wide-spread regimes continue to trade instead of reflexively
      backing off too much.
    """

    # Lower warmup because the linear fair gives a reliable early-session prior.
    WARMUP_TICKS = 8

    # Research-derived linear fair overlay.
    LINEAR_FAIR_SLOPE_PER_TS = 0.0010006
    LINEAR_FAIR_WEIGHT = 0.55
    LINEAR_FAIR_CLIP = 14.0

    # Inventory policy: persistent long bias, but still capped tightly.
    TARGET_INVENTORY_MAX = 52
    SOFT_POSITION_LIMIT = 58
    HARD_POSITION_LIMIT = 78
    MAX_PASSIVE_SIZE = 20

    BASELINE_LONG_TARGET = 12
    WEAK_BASELINE_LONG_TARGET = 8

    # Carry floor should keep us long through the persistent trend unless the
    # microstructure turns sharply adverse.
    CARRY_MODE_SIGNAL = 1.45
    CARRY_MODE_IMBALANCE = 0.08
    CARRY_MODE_FLOW_FLOOR = -0.14
    CARRY_FLOOR_FRACTION = 0.58

    # Don't over-penalize wide spreads; research says signal still exists there.
    WIDE_SPREAD_THRESHOLD = 18

    def load_state(self, trader_data: str) -> Dict[str, List[float] | float | None]:
        state = super().load_state(trader_data)
        p = state.setdefault(self.PRODUCT, {})
        p.setdefault("session_open_mid", None)
        p.setdefault("last_timestamp", None)
        return state

    def _reset_session_state(self, product_state: Dict, mid: Optional[float], timestamp: int) -> None:
        product_state["ema_fast"] = mid
        product_state["ema_slow"] = mid
        product_state["mids"] = [mid] if mid is not None else []
        product_state["flows"] = []
        product_state["prev_best_bid"] = None
        product_state["prev_best_ask"] = None
        product_state["session_open_mid"] = mid
        product_state["last_timestamp"] = timestamp

    def _prepare_session_state(self, product_state: Dict, mid: Optional[float], timestamp: int) -> None:
        last_timestamp = product_state.get("last_timestamp")
        new_session = last_timestamp is None or timestamp < int(last_timestamp) or timestamp == 0
        if new_session:
            self._reset_session_state(product_state, mid, timestamp)
        elif product_state.get("session_open_mid") is None and mid is not None:
            product_state["session_open_mid"] = mid
        product_state["last_timestamp"] = timestamp
    ...
    def build_fair_base(
        ...
        learned_fair = super().build_fair_base(mid, ema_fast, ema_slow, trend_slope)
        session_open_mid = getattr(self, "_session_open_mid", None)
        timestamp = getattr(self, "_session_timestamp", None)

        linear_fair: Optional[float] = None
        if session_open_mid is not None and timestamp is not None:
            linear_fair = float(session_open_mid) + self.LINEAR_FAIR_SLOPE_PER_TS * float(timestamp)

        if learned_fair is None:
            return linear_fair
        if linear_fair is None:
            return learned_fair

        clipped_linear = learned_fair + self.clamp(
            linear_fair - learned_fair,
            -self.LINEAR_FAIR_CLIP,
            self.LINEAR_FAIR_CLIP,
        )
        return (
            self.LINEAR_FAIR_WEIGHT * clipped_linear
            + (1.0 - self.LINEAR_FAIR_WEIGHT) * learned_fair
        )
    ...
    def desired_inventory(
        ...
        if base_target > 0:
            return base_target
        if trend_signal > 0.45 and l2_imbalance > -0.12:
            return self.WEAK_BASELINE_LONG_TARGET
        return base_target
    ...
    def carry_inventory_floor(
        ...
        if base_floor > 0:
            return base_floor
        if trend_signal > 0.85 and l2_imbalance > -0.08 and flow_recent > -0.16:
            return self.WEAK_BASELINE_LONG_TARGET
        return 0
    ...
    def trade_pepper(
        ...
        mid = self.compute_mid(best_bid, best_ask)
        self._prepare_session_state(product_state, mid, int(timestamp))

        self._session_timestamp = int(timestamp)
        self._session_open_mid = product_state.get("session_open_mid")
        try:
            return super().trade_pepper(order_depth, position, state_store, market_trades)
        finally:
            self._session_timestamp = None
            self._session_open_mid = None

class Trader:
    """
    Round 1 model_D:
    - ASH reuses the proven stationary block from model_C unchanged.
    - PEPPER extends model_C with a cleaner research-aligned moving fair.
    """
    ...
    def _load_state(self, trader_data: str) -> Dict:
        ...
        try:
            ...
            pep.setdefault("session_open_mid", None)
            pep.setdefault("last_timestamp", None)
            return parsed
        except Exception:
            return default
