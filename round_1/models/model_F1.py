...
class _PepperTrader(_PepperTraderBase):
    """
    F1: PEPPER core carry sleeve + tactical sleeve.

    Core sleeve keeps structural drift exposure.
    Tactical sleeve uses book support and pullbacks without touching the whole book.
    """
    ...
    CORE_END = 30
    TACTICAL_MAX = 24
    TACTICAL_BOOK_BONUS = 8
    TACTICAL_PULLBACK_Z = -0.60
    CORE_SELL_PROTECT = 0.18
    ...
    def _core_target(self, l2_imbalance: float, flow_recent: float) -> int:
        p = self._session_progress()
        if p < self.EARLY_END:
            base = self.CORE_EARLY
        elif p < self.MID_END:
            base = self.CORE_MID
            ...
            base = self.CORE_END
        if l2_imbalance < -0.18 and flow_recent < -0.12:
            base -= 8
        elif l2_imbalance < -0.10:
            base -= 4
        return max(8, min(self.TARGET_INVENTORY_MAX, base))
    ...
    def desired_inventory(
        ...
        raw_signal = trend_signal + 0.8 * continuation_adj
        core = self._core_target(l2_imbalance, flow_recent) if raw_signal > -0.55 else max(10, self._core_target(l2_imbalance, flow_recent) - 12)
        supportive = 0
        ...
        tactical = 0
        if raw_signal > -0.15:
            tactical = min(self.TACTICAL_MAX, max(0, int(round(abs(raw_signal) * 5.0))))
            if l2_imbalance > 0.04:
                tactical += 4
            if supportive >= 2:
                tactical += self.TACTICAL_BOOK_BONUS
        if visible_spread is not None and visible_spread >= self.WIDE_SPREAD_THRESHOLD:
            tactical = max(0, tactical - 4)
        target = min(self.TARGET_INVENTORY_MAX, core + min(self.TACTICAL_MAX, tactical))
        self._last_core_target = core
        self._last_tactical_target = max(0, target - core)
        return target
    ...
    def carry_inventory_floor(
        ...
        core = self._core_target(l2_imbalance, flow_recent)
        if trend_signal + 0.8 * continuation_adj < -0.75:
            core = max(8, core - 10)
        return min(target_inventory, core)
    ...
    def passive_sizes(
        ...
        passive_buy_size, passive_sell_size = super().passive_sizes(
            position, target_inventory, buy_capacity, sell_capacity,
            buy_hard_capacity, sell_hard_capacity, vol, visible_spread,
            trend_signal, continuation_adj,
        )
        core = int(getattr(self, '_last_core_target', 0))
        tactical = int(getattr(self, '_last_tactical_target', 0))
        if position < core:
            passive_sell_size = int(math.floor(passive_sell_size * self.CORE_SELL_PROTECT))
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 3)
        elif position < core + tactical:
            passive_sell_size = int(math.floor(passive_sell_size * 0.55))
    ...
    def adjust_directional_quotes(
        ...
        passive_bid, passive_ask, passive_buy_size, passive_sell_size = super().adjust_directional_quotes(
            passive_bid, passive_ask, passive_buy_size, passive_sell_size,
            best_bid, best_ask, visible_spread, signal_ticks, trend_signal,
            l2_imbalance, micro_shift, continuation_adj, flow_recent,
            buy_capacity, sell_capacity, buy_hard_capacity, sell_hard_capacity,
        )
        core = int(getattr(self, '_last_core_target', 0))
        tactical = int(getattr(self, '_last_tactical_target', 0))
        total_target = core + tactical
        gap = max(0, total_target - int(getattr(self, '_position_snapshot', 0)))
        if gap > 0 and best_bid is not None and best_ask is not None and visible_spread is not None and visible_spread >= 4:
            if l2_imbalance > 0.04 and continuation_adj > -0.10:
                passive_bid = max(passive_bid, min(best_ask - 1, best_bid + 1))
        if int(getattr(self, '_position_snapshot', 0)) < core:
            passive_ask += 1
    ...
    def trade_pepper(
        ...
        self._last_core_target = 0
        self._last_tactical_target = 0
        try:
...
class Trader:
    """Round 1 model_F1: new PEPPER strategy idea variant."""
