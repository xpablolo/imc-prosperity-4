...
class _PepperTrader(_PepperTraderBase):
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            if progress < self.EARLY_END:
                ...
                bonus_scale = 0.40
                sell_scale = 0.48
            bonus = int(round(min(9, gap / 7.0 * bonus_scale)))
    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            if abs(raw_signal) <= self.HYSTERESIS_NEUTRAL_SIGNAL and intention < self.HYSTERESIS_STRONG_INTENTION and gap < self.HYSTERESIS_SOFT_GAP:
            ...
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if intention >= 3 and gap >= 12 else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and (gap >= self.HYSTERESIS_SOFT_GAP or intention >= self.HYSTERESIS_STRONG_INTENTION):
                passive_ask += 1
            if intention >= 3:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= 16 else 2))
                passive_sell_size = int(math.floor(passive_sell_size * (0.14 if progress < self.MID_END else 0.42)))
            elif intention <= 1:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
                passive_sell_size = int(math.floor(passive_sell_size * (0.30 if progress < self.MID_END else 0.60)))
            else:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
                passive_sell_size = int(math.floor(passive_sell_size * (0.20 if progress < self.MID_END else 0.50)))
...
class Trader:
    """Round 1 model_E_plus_10: PEPPER hysteresis / anti-flicker refinement."""
