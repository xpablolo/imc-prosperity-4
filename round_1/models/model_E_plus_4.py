...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_4: PEPPER execution and order-placement refinement.
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            if progress < self.EARLY_END:
                ...
                bonus_scale = 0.32
                sell_scale = 0.56
            bonus = int(round(min(7, gap / 8.5 * bonus_scale)))
            ...
            if gap >= self.UNDERINVESTED_GAP_GUARD:
                passive_sell_size = int(math.floor(passive_sell_size * 0.45))
    ...
    def adjust_directional_quotes(
        ...
        favorable_book = (
            l2_imbalance > self.FAVORABLE_L2_LEVEL
            or continuation_adj > 0.18
            or micro_shift > 0.12
        )
        ...
        if raw_signal > -0.20 and gap > 0:
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if (gap >= 14 and favorable_book) else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END:
                passive_ask += 1
            if progress < self.EARLY_END and gap >= 12 and favorable_book:
                passive_ask += 1
            size_bonus = 2 if favorable_book and gap >= 16 else 0 if neutral_book else 1
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + size_bonus)
            sell_scale = 0.22 if progress < self.EARLY_END else 0.42 if progress < self.MID_END else 0.66
            if neutral_book:
                sell_scale += 0.08
            passive_sell_size = int(math.floor(passive_sell_size * min(0.90, sell_scale)))
