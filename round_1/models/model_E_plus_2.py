...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_2: stronger reluctance to sell while PEPPER is underinvested.
    ...
    LATE_SIGNAL_BIAS = 0.45
    EARLY_LONG_FLOOR = 76
    MID_LONG_FLOOR = 64
    LATE_LONG_FLOOR = 50
    ENDGAME_LONG_FLOOR = 28

    UNDERINVESTED_GAP_GUARD = 12
    UNDERINVESTED_SELL_SCALE = 0.45
    ...
    def carry_inventory_floor(
        ...
        progress = self._session_progress()
        floor = base_floor
        if raw_signal > -0.55 and progress < self.LATE_END:
            floor = max(floor, self._session_long_floor(l2_imbalance, flow_recent))
        if target_inventory > 0 and progress < self.LATE_END:
            reluctance_floor = max(0, target_inventory - self.UNDERINVESTED_GAP_GUARD)
            if raw_signal > -0.30:
                floor = max(floor, reluctance_floor)
        return floor
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            if progress < self.EARLY_END:
                ...
                bonus_scale = 0.35
                sell_scale = 0.60
            bonus = int(round(min(8, gap / 8.0 * bonus_scale)))
