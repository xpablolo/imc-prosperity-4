...
class _PepperTrader(_PepperTraderBase):
    """G4: properly stateful PEPPER state machine."""
    ...
    def _next_state(self, prev_state: str, position: int, l2_imbalance: float, continuation_adj: float, flow_recent: float) -> str:
        p = self._session_progress()
        supportive = (l2_imbalance > -0.04 and continuation_adj > -0.10)
        adverse = (l2_imbalance < -0.14 and flow_recent < -0.08)
        very_adverse = (l2_imbalance < -0.22 and flow_recent < -0.12)
        if prev_state == 'BUILD':
            if position >= 72:
                return 'HOLD'
            if very_adverse and p > self.EARLY_END:
                return 'DEFEND'
            return 'BUILD'
        if prev_state == 'HOLD':
            if very_adverse and position > 62:
                return 'TRIM'
            if adverse and position > 56:
                return 'DEFEND'
            if position < 58 and supportive:
                return 'BUILD'
            return 'HOLD'
        if prev_state == 'DEFEND':
            if supportive and position < 64:
                return 'BUILD'
            if not adverse:
                return 'HOLD'
            return 'DEFEND'
        if prev_state == 'TRIM':
            if supportive and position < 60:
                return 'BUILD'
            if not very_adverse:
                return 'HOLD'
            return 'TRIM'
        return 'BUILD' if p < self.MID_END else 'HOLD'
    ...
    def desired_inventory(self, trend_signal: float, continuation_adj: float, visible_spread: int | None, vol: float, l2_imbalance: float, flow_recent: float) -> int:
        state = getattr(self, '_policy_state', 'HOLD')
        target = {
            'BUILD': self.BUILD_TARGET,
            'HOLD': self.HOLD_TARGET,
            'DEFEND': self.DEFEND_TARGET,
            'TRIM': self.TRIM_TARGET,
        }[state]
        raw_signal = trend_signal + 0.8 * continuation_adj
        if raw_signal < -0.90:
            target -= 10
        elif raw_signal > 0.60 and state in {'BUILD', 'HOLD'}:
            target += 2
        self._last_state_target = max(12, min(self.TARGET_INVENTORY_MAX, target))
        return self._last_state_target
    ...
    def carry_inventory_floor(self, trend_signal: float, continuation_adj: float, l2_imbalance: float, flow_recent: float, target_inventory: int) -> int:
        state = getattr(self, '_policy_state', 'HOLD')
        floor_frac = {'BUILD': 0.90, 'HOLD': 0.84, 'DEFEND': 0.72, 'TRIM': 0.56}[state]
        floor = max(12, int(round(target_inventory * floor_frac)))
    ...
    def passive_sizes(self, position: int, target_inventory: int, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int, vol: float, visible_spread: int | None, trend_signal: float, continuation_adj: float) -> tuple[int, int]:
        ...
        state = getattr(self, '_policy_state', 'HOLD')
        if state == 'BUILD':
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 4)
            passive_sell_size = int(math.floor(passive_sell_size * 0.12))
        elif state == 'HOLD':
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
            passive_sell_size = int(math.floor(passive_sell_size * 0.30))
        elif state == 'DEFEND':
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
            passive_sell_size = int(math.floor(passive_sell_size * 0.65))
        elif state == 'TRIM':
            passive_sell_size = min(sell_capacity, sell_hard_capacity, passive_sell_size + 3)
            passive_buy_size = int(math.floor(passive_buy_size * 0.55))
        return max(0, passive_buy_size), max(0, passive_sell_size)
    ...
    def adjust_directional_quotes(self, passive_bid: int, passive_ask: int, passive_buy_size: int, passive_sell_size: int, best_bid: int | None, best_ask: int | None, visible_spread: int | None, signal_ticks: float, trend_signal: float, l2_imbalance: float, micro_shift: float, continuation_adj: float, flow_recent: float, buy_capacity: int, sell_capacity: int, buy_hard_capacity: int, sell_hard_capacity: int) -> tuple[int, int, int, int]:
        ...
        state = getattr(self, '_policy_state', 'HOLD')
        if best_bid is not None and best_ask is not None and visible_spread is not None and visible_spread >= 4:
            if state == 'BUILD':
                passive_bid = max(passive_bid, min(best_ask - 1, best_bid + 1))
                passive_ask += 1
            elif state == 'TRIM':
                passive_ask = min(passive_ask, max(best_bid + 1, best_ask - 1))
            elif state == 'DEFEND' and l2_imbalance < -0.10:
                passive_ask = min(passive_ask, max(best_bid + 1, best_ask - 1))
    ...
    def trade_pepper(self, order_depth: OrderDepth, position: int, state_store: Dict, market_trades: List[Trade], timestamp: int) -> List[Order]:
        ...
        best_bid, best_bid_volume, best_ask, best_ask_volume = self.best_levels(order_depth)
        mid = self.compute_mid(best_bid, best_ask)
        _l1, l2, _micro = self.depth_features(order_depth, mid, best_bid_volume, best_ask_volume)
        pep_state = state_store.setdefault(self.PRODUCT, {'policy_state': 'BUILD'})
        prev_state = str(pep_state.get('policy_state', 'BUILD'))
        flow_recent = float(pep_state.get('flows', [])[-1]) if pep_state.get('flows') else 0.0
        continuation_adj = flow_recent * self.CONTINUATION_BONUS if abs(flow_recent) >= self.CONTINUATION_IMBALANCE else 0.0
        state = self._next_state(prev_state, position, l2, continuation_adj, flow_recent)
        pep_state['policy_state'] = state
        self._policy_state = state
        self._last_state_target = 0
        try:
            return super().trade_pepper(order_depth, position, state_store, market_trades)
        finally:
            self._current_progress = 1.0
            self._policy_state = 'HOLD'


class Trader:
    """Round 1 model_G4: policy architecture research variant."""
