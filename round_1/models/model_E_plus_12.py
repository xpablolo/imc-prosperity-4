    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            ...
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if strong_support and gap >= 10 else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and supportive_book:
                passive_ask += 1
            if strong_support:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= 16 else 2))
                passive_sell_size = int(math.floor(passive_sell_size * (0.10 if progress < self.MID_END else 0.36)))
            elif supportive_book:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
                passive_sell_size = int(math.floor(passive_sell_size * (0.18 if progress < self.MID_END else 0.46)))
            else:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
                passive_sell_size = int(math.floor(passive_sell_size * (0.32 if progress < self.MID_END else 0.62)))
...
class Trader:
    """Round 1 model_E_plus_12: PEPPER book-intention execution modulation refinement."""
