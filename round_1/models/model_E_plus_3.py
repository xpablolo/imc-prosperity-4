...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_3: combined late-session carry floor + stronger underinvested sell reluctance.
    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if gap >= 12 else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END:
                passive_ask += 1
            if progress < self.EARLY_END and gap >= 10:
                passive_ask += 1
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= 18 else 1))
            sell_scale = 0.15 if progress < self.EARLY_END else 0.35 if progress < self.MID_END else 0.60
