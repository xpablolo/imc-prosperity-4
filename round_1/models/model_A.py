...
class _PepperTrader:
    ...
    TARGET_INVENTORY_MAX = 28
    INVENTORY_SKEW = 0.17
    SOFT_POSITION_LIMIT = 42
    HARD_POSITION_LIMIT = 68
    MAX_PASSIVE_SIZE = 16
    ...
    CARRY_MODE_SIGNAL = 2.20
    CARRY_MODE_IMBALANCE = 0.18
    CARRY_MODE_FLOW_FLOOR = 0.04
    CARRY_FLOOR_FRACTION = 0.30
    CARRY_OPPOSITE_SIZE_SCALE = 0.60
    CARRY_EXTRA_QUOTE_WIDEN = 1
    CARRY_SIZE_BONUS = 1

    BUDGET_MIN_UNITS = 18
    BUDGET_MAX_UNITS = 48
    BUDGET_BASE_RISK = 360.0
    BUDGET_TREND_BONUS = 0.40
    BUDGET_ADVERSE_VOL = 1.95
    BUDGET_ADVERSE_RESID = 2.80
    BUDGET_ADVERSE_SPREAD = 0.34
    ...
    def inventory_budget_units(
        ...
        aligned_strength = (
            abs(trend_signal)
            + 0.65 * abs(continuation_adj)
            + 1.40 * aligned_imbalance
            + 0.90 * aligned_flow
        )
        risk_budget = self.BUDGET_BASE_RISK * (
            1.0 + self.BUDGET_TREND_BONUS * self.clamp(aligned_strength / 4.0, 0.0, 1.0)
        )
        adverse_move = 4.0 + self.BUDGET_ADVERSE_VOL * vol + self.BUDGET_ADVERSE_RESID * max(0.0, abs(resid_z) - 0.5)
    ...
    def trade_pepper(
        ...
        target_scale = inventory_budget / max(1.0, float(self.TARGET_INVENTORY_MAX))
        target_inventory = int(round(target_inventory * target_scale))
