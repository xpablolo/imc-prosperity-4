...
class _PepperTrader(_PepperTraderBase):
    """
    E_plus_1: slightly higher late-session PEPPER carry floor.
    ...
    LATE_SIGNAL_BIAS = 0.50
    EARLY_LONG_FLOOR = 76
    MID_LONG_FLOOR = 64
    LATE_LONG_FLOOR = 56
    ENDGAME_LONG_FLOOR = 34
    ...
    def carry_inventory_floor(
        ...
        raw_signal = trend_signal + 0.8 * continuation_adj + self._inventory_bias()
        if raw_signal > -0.55 and self._session_progress() < self.LATE_END:
            return max(base_floor, self._session_long_floor(l2_imbalance, flow_recent))
        return base_floor
