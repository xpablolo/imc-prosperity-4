...
class Trader:
    ...
    def load_state(self, trader_data: str) -> Dict[str, List[float] | float | None]:
        default = {"ema_fast": None, "ema_slow": None, "mids": [], "flows": []}
        if not trader_data:
    ...
    def desired_inventory(self, trend_signal: float, continuation_adj: float) -> int:
        direction = self.sign(trend_signal + 0.8 * continuation_adj)
        if direction == 0:
            return 0
        strength = self.clamp(abs(trend_signal) / 3.5 + 0.45 * abs(continuation_adj), 0.0, 1.0)
        return int(round(direction * self.TARGET_INVENTORY_MAX * strength))
    ...
    def trade_pepper(
        ...
        product_state = state_store.setdefault(
            self.PRODUCT,
            {"ema_fast": None, "ema_slow": None, "mids": [], "flows": []},
        )

        current_flow = self.signed_trade_flow(market_trades, best_bid, best_ask, mid)
        ...
        target_inventory = self.desired_inventory(trend_signal, continuation_adj)
        ...
        if order_depth.buy_orders and sell_capacity > 0 and sell_hard_capacity > 0:
            ...
            if signal_ticks > self.DIRECTIONAL_SIGNAL and projected_position < target_inventory:
                remaining_take = min(remaining_take, max(4, self.AGGRESSIVE_CAP // 3))
