...
class _PepperTrader(_PepperTraderBase):
    ...
    INTENTION_CONT = 0.16
    MICRO_HYSTERESIS_SIGNAL = 0.08
    MICRO_HYSTERESIS_GAP = 14
    ...
    def passive_sizes(
        ...
        if target_inventory > 0 and gap > 0:
            ...
            bonus = int(round(min(8, (1.5 + 6.5 * gap_ratio) * bonus_scale)))
            passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + bonus)
            taper = max(0.0, 1.0 - gap_ratio * (1.0 - sell_scale))
            passive_sell_size = int(math.floor(passive_sell_size * taper))
            if gap >= self.UNDERINVESTED_GAP_GUARD:
                passive_sell_size = int(math.floor(passive_sell_size * (self.UNDERINVESTED_SELL_SCALE + 0.03)))
    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            if abs(raw_signal) <= self.MICRO_HYSTERESIS_SIGNAL and intention < 3 and gap < self.MICRO_HYSTERESIS_GAP:
            ...
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                extra_step = 1 if intention >= 3 and gap >= 10 else 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and (gap >= self.MICRO_HYSTERESIS_GAP or intention >= 3):
                passive_ask += 1
            if intention >= 3:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= 16 else 2))
                passive_sell_size = int(math.floor(passive_sell_size * (0.12 if progress < self.MID_END else 0.40)))
            elif intention <= 1:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 1)
                passive_sell_size = int(math.floor(passive_sell_size * (0.28 if progress < self.MID_END else 0.58)))
            else:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + 2)
                passive_sell_size = int(math.floor(passive_sell_size * (0.18 if progress < self.MID_END else 0.48)))
...
class Trader:
    """Round 1 model_E_plus_15: PEPPER refresh / hysteresis micro-refinement."""
