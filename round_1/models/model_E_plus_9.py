    ...
    def adjust_directional_quotes(
        ...
        if raw_signal > -0.20 and gap > 0:
            if visible_spread is not None and visible_spread >= 4 and best_bid is not None and best_ask is not None:
                if gap >= self.PLACEMENT_GAP_STRONG and intention >= self.PLACEMENT_NEUTRAL_INTENTION:
                    extra_step = 1
                else:
                    extra_step = 0
                target_bid = min(best_ask - 1, best_bid + 1 + extra_step)
                passive_bid = max(passive_bid, target_bid)
            if progress < self.MID_END and gap >= self.PLACEMENT_GAP_SOFT:
                passive_ask += 1
            if intention >= 3:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (3 if gap >= self.PLACEMENT_GAP_STRONG else 2))
                sell_scale = 0.14 if progress < self.MID_END else 0.42
                ...
                sell_scale = 0.34 if progress < self.MID_END else 0.62
                passive_sell_size = int(math.floor(passive_sell_size * sell_scale))
            else:
                passive_buy_size = min(buy_capacity, buy_hard_capacity, passive_buy_size + (2 if gap >= self.PLACEMENT_GAP_SOFT else 1))
                sell_scale = 0.22 if progress < self.MID_END else 0.50
                passive_sell_size = int(math.floor(passive_sell_size * sell_scale))
...
class Trader:
    """Round 1 model_E_plus_9: PEPPER placement-offset refinement on top of E_plus_6."""
