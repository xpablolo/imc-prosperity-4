...
# =============================================================================
# model_v3 — keeps the structure of model_v2, but changes how execution risk is
# handled so the behavior is easier to defend from the round 0 data.
#
#   1. EMERALDS still uses a fixed fair of 10000, but removes hard clearing.
#      Fair-price taking is now strictly for inventory repair: it never opens a
#      fresh neutral position, it never flips through zero, and it only engages
#      when inventory is already leaning the wrong way or same-price liquidity
#      on the opposite side is unusually supportive.
#      Passive quoting is managed with inventory skew and side-sensitive sizing.
#
#   2. TOMATOES keeps the dual-EMA core from v2/v3, but now adds a small amount
#      of round-0-specific microstructure information that the raw data supports:
#        - L2 book pressure matters more than L1, so the fair uses a light
#          L1 + L2 imbalance blend plus a simple two-level microprice shift.
#        - A tiny regime overlay separates continuation from mean reversion:
#            * continuation only gets a bonus when deeper book pressure aligns
#              with recent signed flow or with the fast/slow EMA trend;
#            * mean reversion only fades stretched prices when L2 does not
#              confirm the move.
#        - Quote placement becomes more directional only when spread compresses
#          into the 8-10 zone and the signal is already strong.
#        - Aggressive taking is hardened in that same compressed-spread bucket
#          unless the micro signal is clearly confirmed.
#
#   3. The design intentionally stays moderate. Round 0 TOMATOES already has a
#      structurally wide spread, so this file does NOT treat a 13-14 spread as
#      abnormal and does NOT add a large dynamic capacity penalty.
# =============================================================================
...
class Trader:
    ...
    # v3.1 adjustment: keep guardrails simple and fixed. The earlier dynamic
    # version keyed these to visible spread and conviction, which made capacity
    # collapse in a market whose normal spread is already wide (~13 ticks).
    TOMATOES_SOFT_POSITION_LIMIT = 60
    TOMATOES_HARD_POSITION_LIMIT = 72
    TOMATOES_PASSIVE_REDUCTION_STEP = 4

    # Round-0-specific microstructure enrichments.
    ...
    def load_state(self, trader_data: str) -> Dict:
        default = {"ema": None, "ema_fast": None, "ema_slow": None, "mids": [], "flows": []}
        if not trader_data:
    ...
    def tomatoes_continuation_adjustment(
        ...
        aligned_flow = flow_recent * l2_imbalance > 0.02
        ...
        strength = self.clamp(abs(l2_imbalance) / 0.35 + 0.35 * abs(flow_recent), 0.0, 1.0)
    ...
    def adjust_tomatoes_directional_quotes(
        ...
        # Toxicity filter: when deeper book pressure is directional, reduce the
        # passive quote that is most likely to be adversely selected.
    ...
    # TOMATOES strategy
    # =========================
    def trade_tomatoes(self, product: str, order_depth: OrderDepth, position: int, state_store: Dict, market_trades: List[Trade]) -> List[Order]:
        orders: List[Order] = []
        ...
        continuation_adj = self.tomatoes_continuation_adjustment(l2_imbalance, flow_recent, ema_fast, ema_slow)
        ...
        # 2) Passive quotes
        # Keep the useful v3 behavior: re-anchor on projected post-take position.
